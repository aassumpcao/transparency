...
power <- function(n = 5570, alpha = .1, H0 = 0, H1 = .05, sig = 1) {
  ...
  y_barcv <- z_alpha * (sig / sqrt(n)) + H0
  # Calculate the power under H1.
  power <- pnorm(q = y_barcv, mean = H1, sd = sig / sqrt(n), lower.tail = FALSE)
  # Report the power.
  power
}
...
# find 90% power using two-sided .05 alpha
# n = 4203
...
abline(h = .9,   lty = 1, lwd = 2, col = 'red')
abline(v = 4203, lty = 1, lwd = 2, col = 'red')
...
# include legend
legend(x = 3000, y = .4,  col = c('black' , 4, 'grey'), cex = .6, lwd = 2,
  legend = c('two-sided alpha = .1', 'two-sided alpha = .05',
  'two-sided alpha = .01'))
